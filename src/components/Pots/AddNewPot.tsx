...
import { addPotAction } from "@/actions/pots";

const MAX_NAME_LENGTH = 30;

export function AddNewPot({ usedColors = [] }: { usedColors?: string[] }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    setError(null);
    const result = await addPotAction({
      name,
          ...
          <DialogFooter className="w-full">
            <Button className="w-full" type="submit" disabled={loading}>
              <span className="flex items-center gap-2">
                ...
                <span>{loading ? "Adding" : "Add Pot"}</span>
              </span>
            </Button>
          </DialogFooter>
        </form>
