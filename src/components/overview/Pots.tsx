import Image from "next/image";
import Link from "next/link";

export default function Pots() {
  return (
    <div className="area-pots bg-background rounded-12 grid grid-cols-1 gap-5 px-5 py-6 sm:p-8">
      <div className="flex items-center justify-between">
        <h2 className="text-primary text-xl font-bold">Pots</h2>
        <Link
          className="group hover:text-primary text-muted-foreground flex items-center gap-3 text-sm capitalize transition-colors"
          href="/pots"
        >
          See details
          <svg
            width="5"
            height="9"
            viewBox="0 0 5 9"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              d="M0.640312 0.109986L4.39031 3.85999C4.42518 3.89481 4.45284 3.93617 4.47171 3.9817C4.49058 4.02722 4.50029 4.07602 4.50029 4.1253C4.50029 4.17458 4.49058 4.22338 4.47171 4.2689C4.45284 4.31443 4.42518 4.35578 4.39031 4.39061L0.640313 8.14061C0.587867 8.19312 0.521022 8.22888 0.44824 8.24337C0.375458 8.25787 0.300012 8.25044 0.231454 8.22203C0.162895 8.19363 0.104307 8.14552 0.063105 8.08379C0.0219034 8.02207 -5.82985e-05 7.94951 9.97705e-08 7.8753L-2.28065e-07 0.375299C-5.86328e-05 0.301088 0.0219031 0.228528 0.0631046 0.166805C0.104306 0.105083 0.162895 0.0569735 0.231453 0.0285664C0.300012 0.000158297 0.375458 -0.00726797 0.44824 0.00722597C0.521022 0.0217199 0.587867 0.0574817 0.640312 0.109986Z"
              fill="currentColor"
            />
          </svg>
        </Link>
      </div>

      <div className="@container">
        <div className="grid gap-5 @lg:grid-cols-2">
          <div className="bg-secondary rounded-12 flex min-w-0 items-center gap-5.5 p-5">
            <Image
              className="h-fit w-auto"
              width={20}
              height={20}
              alt=""
              src="/icons/icon-pot.svg"
            />

            <div className="flex flex-col gap-3">
              <p className="text-muted-foreground text-sm">Total Saved</p>
              <p className="text-primary text-32 leading-[1.2] font-bold">
                $850
              </p>
            </div>
          </div>

          <div className="grid grid-cols-2 gap-4">
            <div className="flex items-center gap-4">
              <div className="bg-chart-1 w-1 self-stretch"></div>
              <div className="flex flex-col gap-1">
                <span className="text-muted-foreground text-xs">Savings</span>
                <span className="text-primary text-sm font-bold">$159</span>
              </div>
            </div>

            <div className="flex items-center gap-4">
              <div className="bg-chart-2 w-1 self-stretch"></div>
              <div className="flex flex-col gap-1">
                <span className="text-muted-foreground text-xs">Gift</span>
                <span className="text-primary text-sm font-bold">$40</span>
              </div>
            </div>

            <div className="flex items-center gap-4">
              <div className="bg-chart-4 w-1 self-stretch"></div>
              <div className="flex flex-col gap-1">
                <span className="text-muted-foreground text-xs">
                  Concert Ticket
                </span>
                <span className="text-primary text-sm font-bold">$110</span>
              </div>
            </div>

            <div className="flex items-center gap-4">
              <div className="bg-chart-3 w-1 self-stretch"></div>
              <div className="flex flex-col gap-1">
                <span className="text-muted-foreground text-xs">
                  New Laptop
                </span>
                <span className="text-primary text-sm font-bold">$10</span>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}
